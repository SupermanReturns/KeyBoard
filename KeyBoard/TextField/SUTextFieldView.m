...
@interface  SUTextFieldView()<KeyboardViewDelegate>

@property(nonatomic,strong) KeyboardView *numberKeyboardView;
@property(nonatomic,strong) KeyboardView *abcKeyboardView;
@property(nonatomic,strong) KeyboardView *otherKeyboardView;
@property(nonatomic,strong) MenuSegView *menuSgView;


@end

@implementation SUTextFieldView
...
-(void)addtextView
{
    ...
    if (isDefaultKeyboard) {
        MenuSegView *toolView = [[MenuSegView alloc] initWithFrame:CGRectMake(0, 0, KScreenWidth, 40) TitleArray:@[@"系统键盘",@"数字",@"字母",@"符号"]];
...
-(void)textFiledDidChangeText :(SUTextFieldView *)textField;
{
...
-(void)addNumberBoard
{
    if (!_numberKeyboardView) {
        _numberKeyboardView = [[KeyboardView alloc] initWithFrame:self.keyBgBoardView.bounds keyboardType:NumberKeyboard];
...
{
    if (!_abcKeyboardView) {
        _abcKeyboardView = [[KeyboardView alloc] initWithFrame:self.keyBgBoardView.bounds keyboardType:type];
...
-(void)addOtherKeyboardView
{
    if (!_otherKeyboardView) {
        _otherKeyboardView = [[KeyboardView alloc] initWithFrame:self.keyBgBoardView.bounds keyboardType:otherKeyboard];
